...
class NvSwitchAttestationTestRemote(TestCase):
    ...
    def test_switch_remote_attest_successful(
        ...
        result, jwt_token = attest_nvswitch_remote.attest(
            nonce, switch_evidence_list, { "verifier_url": "http://localhost:5000" }
        )
    ...
    @mock.patch("requests.get")
    @mock.patch("requests.request")
    def test_switch_remote_attest_with_service_key_successful(
        self, nras_mock_request, jwks_mock_request
    ):
        header = {"kid": "nv-eat-kid-test-1234"}
        with open(overall_claims_file_path, "r") as file:
        ...
        nonce = "931d8dd0add203ac3d8b4fbde75e115278eefcdceac5b87671a748f32364dfcb"
        result, jwt_token = attest_nvswitch_remote.attest(
            nonce, switch_evidence_list, {"verifier_url": "http://localhost:5000", "service_key": "someServiceKey"}
        )
        self.assertTrue(result)
        self.assertEqual(jwt_token[0][0], "JWT")
        self.assertEqual(jwt_token[0][1], overall_claims_jwt)
    ...
    @mock.patch("requests.get")
    @mock.patch("requests.request")
    def test_switch_remote_attest_successful_with_claims_version_3(
            self, nras_mock_request, jwks_mock_request
    ):
        header = {"kid": "nv-eat-kid-test-1234"}
        ...
        nonce = "931d8dd0add203ac3d8b4fbde75e115278eefcdceac5b87671a748f32364dfcb"
        result, jwt_token = attest_nvswitch_remote.attest(
            nonce, switch_evidence_list, { "verifier_url": "http://localhost:5000", "claims_version": "3.0" }
        )
        self.assertTrue(result)
        self.assertEqual(jwt_token[0][0], "JWT")
        self.assertEqual(jwt_token[0][1], overall_claims_jwt)
    ...
    @mock.patch("requests.get")
    @mock.patch("requests.request")
    def test_switch_remote_attest_successful_with_claims_version_2(
            self, nras_mock_request, jwks_mock_request
    ):
        header = {"kid": "nv-eat-kid-test-1234"}
        with open(overall_claims_file_path, "r") as file:
        ...
        nonce = "931d8dd0add203ac3d8b4fbde75e115278eefcdceac5b87671a748f32364dfcb"
        result, jwt_token = attest_nvswitch_remote.attest(
            nonce, switch_evidence_list, { "verifier_url": "http://localhost:5000", "claims_version": "2.0" }
        )
        self.assertTrue(result)
        self.assertEqual(jwt_token[0][0], "JWT")
        self.assertEqual(jwt_token[0][1], overall_claims_jwt)
    ...
    @mock.patch("requests.get")
    @mock.patch("requests.request")
    def test_switch_remote_attest_with_service_key_fails_due_to_measurement_mismatch(
        self, nras_mock_request, jwks_mock_request
    ):
        header = {"kid": "nv-eat-kid-test-1234"}
        with open(overall_claims_file_path, "r") as file:
    ...
    def test_build_payload(self):
        ...
        payload = attest_nvswitch_remote.build_payload("nonce", evidence_list, "2.0")
    ...
    @mock.patch("requests.get")
    @mock.patch("requests.request")
    def test_switch_remote_attest_with_service_key_fails_when_nras_call_fails(
        self, nras_mock_request, jwks_mock_request
    ):
        header = {"kid": "nv-eat-kid-test-1234"}
        nras_mock_response = mock.Mock()
    ...
    def test_switch_remote_attest_with_service_key_fails_when_creating_request_due_to_serialization_error(
        self
    ):
        class TestClass:
            pass
